...
if (as_clicked('dopost')) {
	...
	if (!as_check_form_security_code('post', as_post_text('code'))) {
		...
		if (empty($errors)) {
			...
			$recentQuestions = as_db_select_with_pending(as_db_qs_selectspec(null, 'created', 0, null, null, false, true, 5));

			foreach ($recentQuestions as $song) {
				if (!$song['hidden']) {
		...
		if (empty($errors)) {
			...
			$songid = as_song_create($followreview, $userid, as_get_logged_in_handle(), $cookieid,
				$in['number'], $in['songkey'], $in['title'], $in['alias'], $in['content'], $in['format'], $in['text'], isset($in['tags']) ? as_tags_to_tagstring($in['tags']) : '',
...
$as_content['form'] = array(
	...
	'fields' => array(
		...
		'number' => array(
			...
			'<td><input name="number" id="number" autocomplete="off" type="text" value="" class="as-form-wide-number"></td>'.
			'<td><label class="as-form-wide-label">' . as_lang_html('song/q_key_label') .' </label></td>'.
			'<td><input name="songkey" id="songkey" autocomplete="off" type="text" value="" class="as-form-wide-number"></td>'.
			'</tr></table>',
		),
		...
		'alias' => array(
			'label' => as_lang_html('song/q_alias_label'),
			'tags' => 'name="alias" id="alias" autocomplete="off"',
			'value' => as_html(@$in['alias']),
			'error' => as_html(@$errors['alias']),
		),
